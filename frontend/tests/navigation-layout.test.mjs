...
const layoutSource = readFileSync(resolve(root, 'src/components/AppLayout.vue'), 'utf8')

assert.match(routerSource, /component:\s*\(\)\s*=>\s*import\(['"]@\/components\/AppLayout\.vue['"]\)/)
...
for (const label of ['学生管理', '班级管理', '课程管理', '学籍异动', '用户管理']) {
